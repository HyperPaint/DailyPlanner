...
namespace App
{
    ...
    class TaskActivity : AppCompatActivity
    {
        ...
        private TaskBlockAdapter ListAdapter = new TaskBlockAdapter();
        ...
        private void Initialize()
        {
            ...
            ListAdapter.OpenClick += (s, e) => GlobalVariables.TWorker.Open(e, this);
            ListAdapter.CompleteClick += (s, e) =>
            {
                if (GlobalVariables.TWorker.Complete(e))
                {
                    ListAdapter.NotifyItemRangeChanged(0, ListAdapter.ItemCount);
                }
                else
                {
                    ListAdapter.NotifyItemRemoved(e);
                }
            };
            ListAdapter.ChangeClick += (s, e) => { GlobalVariables.TWorker.LastPosition = e; Utility.DisplayTaskForm(this); };
            ListAdapter.DeleteClick += (s, e) => GlobalVariables.TWorker.Delete(e); 
            ListAdapter.AddSubtaskClick += (s, e) => { };
            ListAdapter.ItemClick += (s, e) => { };

            New = FindViewById<ImageButton>(Resource.Id.New);
            New.Click += (s, e) => { GlobalVariables.TWorker.LastPosition = -1; Utility.DisplayTaskForm(this); };
        }
    }
}